...
# 4. MODEL TRAINING COMPONENT - Decision Tree
@dsl.component(
    packages_to_install=["scikit-learn", "joblib"]
)
...
# DEFINE THE PIPELINE WITH ONLY DECISION TREE MODEL
@dsl.pipeline(
...
def diabetes_classification_pipeline(
    ...
    random_state: int = 42,
    dt_max_depth: int = 5,
    # MinIO/Keycloak parameters with hardcoded values
    ...
    # Train only the decision tree model
    ...
    save_dt_metrics_task = save_to_minio(
        ...
        artifact_name="decision_tree_metrics.json"
    )
    ...
    save_dt_cm_plot_task = save_to_minio(
        ...
        run_id=version_id,
        artifact_type="plots",
        artifact_name="decision_tree_confusion_matrix.html"
    )
    ...
    save_dt_roc_plot_task = save_to_minio(
        ...
        run_id=version_id,
        artifact_type="plots",
        artifact_name="decision_tree_roc_curve.html"
    )
    ...
    save_dt_importance_plot_task = save_to_minio(
        ...
        run_id=version_id,
        artifact_type="plots",
        artifact_name="decision_tree_feature_importance.html"
    )
...
if __name__ == "__main__":
    ...
    pipeline_package_path = 'kubeflow/diabetes_dt_classification_pipeline_minio.yaml'
